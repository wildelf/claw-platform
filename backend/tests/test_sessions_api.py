...
class TestCreateSession:
    ...
    async def test_create_session_with_name(self, client):
        """Create session with name returns 200 and session data."""
    ...
    async def test_create_session_without_name(self, client):
        """Create session without name returns 200 with auto-generated name."""
        response = await client.post("/sessions", json={
            "agent_id": "agent-456",
        })
        assert response.status_code == 200
...
class TestListSessions:
    ...
    async def test_list_sessions_with_data(self, client):
        ...
        # Create two sessions
        await client.post("/sessions", json={"agent_id": "agent-1", "name": "Session 1"})
        await client.post("/sessions", json={"agent_id": "agent-2", "name": "Session 2"})
    ...
    async def test_list_sessions_pagination(self, client):
        ...
        # Create 5 sessions
        for i in range(5):
            await client.post("/sessions", json={"agent_id": f"agent-{i}", "name": f"Session {i}"})
