    ...
    <div class="flex items-center h-full px-4">
      ...
      <nav class="hidden md:flex items-center justify-center flex-1 space-x-8">
        <RouterLink
          ...
          active-class="text-[var(--color-primary)] font-medium"
        >
          Dashboard
        </RouterLink>
        <RouterLink
      ...
      >
        <!-- 太阳图标 (light mode) -->
        <svg v-if="theme === 'light'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-[var(--text-secondary)]">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
        </svg>
        <!-- 月亮图标 (dark mode) -->
        <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-[var(--text-muted)]">
      ...
      <div class="flex items-center space-x-4">
        ...
        <template v-else>
          ...
          <RouterLink
            to="/register"
            class="text-sm text-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors"
          >
            Register
          </RouterLink>
        </template>
