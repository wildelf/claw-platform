...
@pytest.mark.asyncio
async def test_process_user_preference_goes_to_user_md(service):
    # Mock LLM decision with user preference
    mock_decision = MagicMock()
    mock_decision.should_nudge = True
    mock_decision.nudge_type = "memory"
    mock_decision.priority = "medium"
    mock_decision.summary = "用户喜欢中文沟通"
    ...
    with patch.object(service.reasoning_judge, 'judge', return_value=mock_decision):
        ...
        assert result.nudge_triggered is True
        assert "USER.md" in result.memory_written
